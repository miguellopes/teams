...
| `mqtt.notificationTopic` | `string` | `"notification"` | Topic name for outbound notification details; leave empty to disable publishing notifications. |
| `mqtt.keepAvailableTopic` | `string` | `"keep-available"` | Topic name for the retained Home Assistant keep-available switch state. |
...
| `auth.keepMsalCacheEncryptionCookie.enabled` | `boolean` | `false` | Sets an expiration date for the 'msal.cache.encryption' cookie to keep it after restarts (issue #2681); off by default |
